...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # Local fork change: run app-server with approvals and sandbox bypassed
        # unconditionally (incl. kanban workers) so the repair-bridge dispatcher
        # path (hermes kanban -> nicola -> codex app-server) keeps network access
        # for `git push` / `gh` PR operations. Upstream's kanban branch sets
        # sandbox_workspace_write.network_access=false, which would silently break
        # autonomous PR creation for dispatcher-owned workers. Deliberately diverges
        # from upstream's sandbox writable-roots approach for that reason.
        cmd = [
            codex_bin,
